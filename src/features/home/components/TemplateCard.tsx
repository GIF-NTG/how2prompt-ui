...
export function TemplateCard({
  ...
  return (
    <div
      ...
      className="group relative flex h-full w-full animate-[fade-slide-up_400ms_ease_backwards] flex-col gap-[0.55rem] rounded-card border border-[#DBDFD3] bg-white p-[1.1rem_1.2rem] text-left transition-[border-color,transform,box-shadow] duration-150 hover:-translate-y-0.5 hover:border-[#3652E0] hover:shadow-[0_12px_24px_-18px_rgba(27,29,27,0.4)] dark:border-[#2C3130] dark:bg-[#1C2024] dark:hover:border-[#8493FF] dark:hover:shadow-[0_12px_24px_-18px_rgba(0,0,0,0.6)]"
    >
      <button
        type="button"
        aria-label={getI18nValue(template.title)}
        onClick={() => onClick?.(template.id)}
        className="absolute inset-0 z-0 cursor-pointer rounded-card active:scale-[0.98] active:duration-75 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#3652E0]"
      />

      <div className="pointer-events-none relative z-[1] flex h-full flex-col gap-[0.55rem]">
        <div className="flex items-start justify-between gap-2">
          ...
          {isSignedIn && (
            <button
              ...
              className={`pointer-events-auto flex h-[1.7rem] w-[1.7rem] flex-shrink-0 cursor-pointer items-center justify-center rounded-lg border transition-colors duration-150 active:scale-90 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#3652E0] ${
                isFavorited
