...
authoritative wire contract) that this feature's four admin clients integrate against,
...
`{ data, meta }` envelope; throws `ApiError` on non-2xx, per
`src/shared/utils/httpClient.ts`). Every mutating endpoint requires the backend's
`hasRole('ADMIN')` check — a `403` from any of these is expected and normal for a
non-admin caller who somehow bypasses the frontend route guard.
...
edit, delete, or merge) — see `research.md` Decision 3. FR-007 (tag CRUD/merge) and
the category-deletion half of FR-006/FR-008 are not implemented against this
contract; the taxonomy screen surfaces this as a known limitation rather than a
broken control.
...
feedback but must treat the `422` as the source of truth, not just its own
pre-check, per Constitution Principle III.
...
backend behavior (the contract doesn't expose a cache-control signal the frontend
needs to act on); the frontend simply displays whatever the endpoint returns per
request, with no client-side cache duplicating server behavior.
...
clients should follow the same `ApiError`-catch-and-map-to-outcome pattern already
used by `authClient.real.ts`.
