...
| `GET`   | `/templates/{id}`        | —                                           | `isFeatured?: boolean` (optional, may be absent) | Read current Featured state for the admin list badge (FR-005) and edit form initial value. |
...
## Field mapping (per research.md Decision 1)
...
- **Write**: `TemplateUpsert.isFeatured` → sent verbatim as `isFeatured` in the
  ...
  `patchMetadata`.
- **Read**: `RawTemplateDetail.isFeatured?: boolean` → mapped to
  `AdminTemplate.isFeatured` via `raw.isFeatured ?? false` in `mapAdminTemplate`
  (defensive default, matching the existing `isFavorited` pattern).
